...
func (c *Catalog) listCommitWorker(ctx context.Context, repository *graveler.RepositoryRecord, paths []PathRecord, commitCache *lru.Cache, in <-chan compareJob, out chan<- compareJobResult) error {
	for {
		select {
		case <-ctx.Done():
			return nil
